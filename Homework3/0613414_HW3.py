...
class DecisionTree():
    ...
    def splitAttr(self, x, info):
        ...
        for col in range(len(x.columns)-1):  # exclude label column
            ...
            for i in range(len(x)):
                ...
                info_p = (i + 1) * left + (train_num - i + 1) * right
                info_p /= train_num
    ...
    def genTree(self, tree):
        ...
        for _iter in range(10):
            ...
            info_gain, attr_i, threshold = self.splitAttr(tree.x, tree.info)
            if info_gain == 0:
                tree.isLeaf = True
                return tree

            # Partition tree
            left, right = self.partiTree(tree, attr_i, threshold)
            if len(left.x) != len(tree.x) and len(right.x) != len(tree.x):
                break
        tree.setting(attr_i, threshold, left, right)
...
class RandomForest():
    ...
    def growTree(self, tree):
        ...
        for _iter in range(3):
            ...
            tdf = tdf.drop(columns=[feature_name[i] for i in delete_feature])

            # Select best split attribute and threshold
            info_gain, attr_index, threshold = self.splitAttr(tdf, tree.info)
            if info_gain == 0:
                tree.isLeaf = True
                return tree
...
if __name__ == "__main__":
    ...
    clf_all_features = RandomForest(n_estimators=10,
